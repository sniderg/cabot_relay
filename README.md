...
The leg matchup card includes a flat-equivalent pace estimate using the Minetti running cost polynomial. The app resamples each OSRM route polyline at about 100m intervals, looks up elevations with Open-Meteo's Copernicus GLO-90 based elevation API, smooths/clips the resulting grades to control DEM noise, clamps displayed adjustments to 20s/km, and falls back to the bundled coarse profiles if live elevation lookup fails.
